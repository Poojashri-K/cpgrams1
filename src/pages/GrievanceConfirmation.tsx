...
export function GrievanceConfirmation() {
  ...
  const reg = params.get("reg") || "CPG/2026/PENDING"
  ...
  const grievance = (() => {
    try {
      return JSON.parse(
        sessionStorage.getItem(`cpgrams-${reg}`) || "null",
      )
    } catch {
      return null
    }
  ...
  async function sendFeedback() {
    ...
    try {
      await submitFeedback(reg, rating, comment)
      setSent(true)
    } catch {
      ...
      <Card className="overflow-hidden">
        <div className="bg-navy p-8 text-center text-white">
          ...
          <p className="mt-2 text-white/75">
            Keep this ID for tracking your grievance.
          </p>

          <p className="mt-5 font-mono text-2xl font-bold text-saffron">
            {reg}
          </p>
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="border-t pt-5">
            <h2 className="font-semibold">
              How was your filing experience?
            </h2>
          ...
          <Link
            className="inline-flex font-semibold text-navy underline"
            to={`/check-status?reg=${encodeURIComponent(reg)}`}
          >
            Track grievance
          </Link>
        </div>
